...
object ScriptManager {
    ...
    /**
     * Releases every polyglot context without unregistering what the scripts contributed.
     *
     * The shutdown path. [unloadAll] removes script modules from the
     * [net.ccbluex.liquidbounce.features.module.ModuleManager], and the client writes
     * `modules.json` after add-ons shut down, so using it here loses every script module's settings.
     */
    fun closeAll() {
        scripts.forEach(PolyglotScript::close)
        ScriptAsyncUtil.TickScheduler.clear()
