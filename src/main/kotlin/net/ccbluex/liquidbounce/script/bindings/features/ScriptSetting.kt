...
/**
 * Backs `setValue(PolyglotValue)`, which `MixinValue` adds to [Value] since the client has no GraalVM
 * to declare it with. Nashorn compatibility resolves `setting.value = x` to it.
 */
@Suppress("UNCHECKED_CAST")
fun <T : Any> Value<T>.setByPolyglot(t: PolyglotValue) {
    runCatching {
        ...
        set(
            when (get()) {
                ...
                else -> error("Unsupported value type ${get()}")
            }
        )
    }.onFailure {
        logger.error("Could not set value, old value: ${get()}, throwable: $it")
    }
}
